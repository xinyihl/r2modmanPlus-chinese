...
<template>
    <SettingsViewWrapper v-show="isVisible">
        <template #title>{{ activeGame.displayName }} 文件夹</template>
        <template #description>
            <p>
                需要游戏目录才能正确放置相应文件。
            </p>
            <p v-if="StorePlatform[activeGame.activePlatform.storePlatform] === StorePlatform.steam">
                如果未正确设置，<code class="code">{{ activeGame.displayName }}</code> 将在没有模组的情况下启动。
            </p>
        </template>
        <div class="setting-column">
            <div class="setting-row">
                <input
                    ...
                    readonly
                />
                <button class="button" @click="changeGameInstallDirectory">更改</button>
                <button class="button" @click="browseDirectory">浏览</button>
            </div>
            <a href="#" class="help-link" @click.prevent.stop="openHelpLink">我不确定这应该是什么</a>
        </div>
    </SettingsViewWrapper>
</template>
