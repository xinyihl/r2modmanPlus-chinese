...
const categories = ['全部', '目录', '配置', '外观', '调试', '模组包', '其他'] as const;
type Category = typeof categories[number];

const activeCategory = ref<Category>('全部');

function isVisible(section: Category): boolean {
    return activeCategory.value === '全部' || activeCategory.value === section;
}
</script>
...
<template>
    <div id="settings-view">
        <Hero
            title="设置"
            :subtitle="`${appName} 的高级选项：${managerVersionNumber.toString()}`"
            heroType="primary"
        />
        <div class="settings-shell">
            ...
            <div class="settings-list">
                    ...
                    <div class="is-shadowless is-square">
                        <div class="no-padding-left card-header-title">
                            <div class="input-group input-group--flex margin-right">
                                <label for="installed-search" class="non-selectable">搜索</label>
                                <DeferredInput
                ...
                <SettingsSection v-if="isVisible('目录')" name="目录">
                ...
                <SettingsSection v-if="isVisible('配置')" name="配置">
                ...
                <SettingsSection v-if="isVisible('模组包')" name="模组包">
                    <ShowDependencyStrings :search-term="searchTerm"/>
                </SettingsSection>

                <SettingsSection v-if="isVisible('其他')" name="其他">
