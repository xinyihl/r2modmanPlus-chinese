            ...
            <div id="no-content" class="content" v-if="resultCount === 0">
                <div>
                    <div>
                        ...
                        <br/>
                        <h3 class="title is-4">未找到与 “{{ filterText }}” 匹配的{{activeTab === GameInstanceType.GAME ? '游戏' : '服务器'}}</h3>
                        <p class="subtitle">请尝试其他游戏名称或关键词。我们可能尚未支持该游戏。</p>
                        <button class="button margin-top icon-button" @click.prevent.stop="requestNewGame">
                            <span>申请新的{{activeTab === GameInstanceType.GAME ? '游戏' : '服务器'}}</span>
            ...
            <template v-if="viewMode === GameSelectionViewMode.LIST">
                <div id="list-content" class="content">
                    ...
                    <div class="request-game margin-bottom" v-if="resultCount > 0">
                        <h3 class="title is-5">找不到你要找的内容？</h3>
                ...
                <div class="content pad--sides pad--top-none">
                    <template v-if="filterText.length === 0">
                        <template v-if="favouriteGameList.length > 0">
                            <GameSelectionSection title="收藏" :count="favouriteGameList.length" :default-open="true">
                        ...
                        <template v-if="nonFavouriteGameList.length > 0">
                            ...
                            <GameSelectionSection
                                :title="`${activeTab === GameInstanceType.GAME ? '游戏' : '服务器'}`"
...
import { EcosystemSupportedGames, GameInstanceType } from '../../model/schema/ThunderstoreSchema';
