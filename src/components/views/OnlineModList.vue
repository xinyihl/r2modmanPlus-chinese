        ...
        <ExpandableCard
            ...
            <template v-slot:title>
                <span v-if="key.isPinned()">
                    <span class="tag is-info margin-right margin-right--half-width"
                          v-tooltip.right="'在 Thunderstore 上被固定'">
                        已固定
                    </span>
            ...
            <template v-slot:other-icons>
                ...
                <span class='card-header-icon' v-if="isThunderstoreModInstalled(key) && !readOnly">
                    <i class='fas fa-check' v-tooltip.left="'模组已安装'"></i>
                </span>
            </template>
            <template v-slot:description>
                <p class='card-timestamp'><strong>最后更新:</strong> {{getReadableDate(key.getDateUpdated())}}</p>
                <p class='card-timestamp'><strong>类别:</strong> {{getReadableCategories(key)}}</p>
            </template>
            <button class='button' v-if="!readOnly" @click='showDownloadModal(key)'>下载</button>
